...
export function computedHandlers(
    ...
    return computed(() => {
        ...
        for (const key of eventHandlerKeys) {
            const handler =
                properties.value[key as keyof EventHandlersProperties]
            if (handler != null) {
...
export function addHandler<
    T extends { [Key in string]?: (e: any) => void },
    K extends keyof T,
>(key: K, target: T, handler: T[K]): void {
    if (handler == null) {
        return
    }
...
export function setupMatrixWorldUpdate(
    ...
    abortableEffect(() => {
        ...
        const updateMatrixWorld = component.updateWorldMatrix.bind(
            component,
            false,
            true,
        )
...
export function setupPointerEvents(
    ...
    abortableEffect(() => {
        component.ancestorsHaveListeners =
            component.ancestorsHaveListenersSignal.value
        component.pointerEvents = component.isVisible.value
            ? component.properties.value.pointerEvents
            : 'none'
        component.pointerEventsOrder =
            component.properties.value.pointerEventsOrder
        component.pointerEventsType =
            component.properties.value.pointerEventsType
    }, component.abortSignal)
    abortableEffect(() => {
        const rootComponent = component.root.value.component
        component.intersectChildren =
            canHaveNonUikitChildren || rootComponent === component

        if (
            !canHaveNonUikitChildren &&
            component.properties.value.pointerEvents === 'none'
        ) {
            return
        }
...
export function fitNormalizedContentInside(
    ...
    if (
        size.value == null ||
        paddingInset.value == null ||
        borderInset.value == null
    ) {
        return
    }
...
export function readReactive<T>(
    value: T | 'initial' | ReadonlySignal<T | 'initial'>,
): T {
...
export function computedBorderInset(
    ...
    return computed(
        () =>
            keys.map(
                (key) => properties.value[key as keyof BaseOutProperties] ?? 0,
            ) as Inset,
    )
}
...
type UnionToIntersection<U> = (U extends any ? (x: U) => void : never) extends (
    ...
    ? I
    : never
type IntersectValues<T extends Record<PropertyKey, unknown>> =
...
export function computeWorldToGlobalMatrix(
    ...
    target.multiplyMatrices(
        rootComponent.parent.matrixWorld,
        rootComponent.matrix,
    )
}
