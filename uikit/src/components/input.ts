...
import { Vector2, Vector2Tuple } from 'three/webgpu'
import {
    BaseOutProperties,
    InProperties,
    WithSignal,
...
> extends Text<OutProperties> {
    ...
    constructor(
        ...
        const selectionHandlers = signal<EventHandlersProperties | undefined>(
            undefined,
        )
        ...
        const caretTransformation = signal<CaretTransformation | undefined>(
            undefined,
        )
        ...
        abortableEffect(
            () => void (caretColor.value = this.properties.value.color),
            this.abortSignal,
        )
        ...
        const parentClippingRect = computed(
            () => this.parentContainer.value?.clippingRect.value,
        )
...
export function setupSelectionHandlers(
    ...
    abortableEffect(() => {
        ...
        let dragState:
            | { startCharIndex: number; pointerId?: number }
            | undefined
        ...
        target.value = {
            onPointerDown: (e) => {
                ...
                const startCharIndex = uvToCharIndex(
                    component,
                    e.uv,
                    instancedTextRef.current,
                    'between',
                )
                dragState = {
            ...
            onPointerMove: (e) => {
                ...
                const direction =
                    dragState.startCharIndex < charIndex
                        ? 'forward'
                        : 'backward'
...
export function createHtmlInputElement(
    ...
    const updateSelection = () => {
        ...
        if (
            current != null &&
            current[0] === selectionStart &&
            current[1] === selectionEnd
        ) {
            return
        }
...
function setupHtmlInputElement(
    ...
    abortableEffect(
        () => void (element.disabled = properties.value.disabled),
        abortSignal,
    )
    abortableEffect(
        () => void (element.tabIndex = properties.value.tabIndex),
        abortSignal,
    )
    abortableEffect(
        () => void (element.autocomplete = properties.value.autocomplete),
        abortSignal,
    )
    abortableEffect(
        () => element.setAttribute('type', properties.value.type),
        abortSignal,
    )
}
