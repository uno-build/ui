...
> extends Component<OutProperties> {
    ...
    constructor(
        ...
        abortableEffect(() => {
            ...
            defaultAspectRatio.value =
                this.boundingBox.value.size.x / this.boundingBox.value.size.y
        }, this.abortSignal)
        ...
        abortableEffect(() => {
            ...
            positionHelper.z -=
                alignmentZMap[this.properties.value.depthAlign] *
                this.boundingBox.value.size.z
        ...
        abortableEffect(() => {
            ...
            applyAppearancePropertiesToGroup(
                ...
                this.inputConfig?.supportFillProperty ??
                    SupportFillPropertyDefault,
            )
            this.root.peek().requestRender?.()
        }, this.abortSignal)

        const remeasureOnChildrenChange =
            this.inputConfig?.remeasureOnChildrenChange ??
            RemeasureOnChildrenChangeDefault
        if (remeasureOnChildrenChange) {
            const onChildrenChanged =
                this.debounceNotifyAncestorsChanged.bind(this)
    ...
    private childUpdateWorldMatrix(
        ...
        if (!(child.parent instanceof Content)) {
            Object3D.prototype.updateWorldMatrix.apply(child, [
                updateParents,
                updateChildren,
            ])
            return
    ...
    private debounceNotifyAncestorsChanged() {
        ...
        this.timeoutRef = setTimeout(
            this.notifyAncestorsChanged.bind(this),
            0,
        ) as any
    }
    ...
    notifyAncestorsChanged() {
        ...
        for (const child of this.children) {
            child.updateMatrixWorld = this.childUpdateWorldMatrix.bind(
                this,
                child,
                false,
                true,
            )
            child.updateWorldMatrix = this.childUpdateWorldMatrix.bind(
                this,
                child,
            )
        }
...
function applyAppearancePropertiesToGroup(
    ...
    const color =
        (supportFillProperty ? properties.value.fill : undefined) ??
