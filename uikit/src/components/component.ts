...
import {
    type Conditionals,
    createConditionals,
} from '../properties/conditional.js'
import {
    BaseOutProperties,
    InProperties,
    Properties,
...
export function resetGlobalProperties(
    properties: InProperties<BaseOutProperties> | undefined,
) {
    currentGlobalProperties = properties
    globalProperties.setLayer(baseLayerIndex, currentGlobalProperties)
}

export function setGlobalProperties(
    properties: InProperties<BaseOutProperties> | undefined,
) {
...
> extends Mesh<BufferGeometry, Material> {
    ...
    readonly maxScrollPosition = signal<Partial<Vector2Tuple>>([
        undefined,
        undefined,
    ])
    ...
    constructor(
        ...
        protected readonly initialClasses?: Array<
            InProperties<BaseOutProperties> | string
        >,
        config?: {
            material?: Material
        ...
        const updateParentState = () => {
            this.parentContainer.value =
                this.parent instanceof Component
                    ? (this.parent as Container)
                    : undefined
            ;(
                this.properties as PropertiesImplementation<OutProperties>
            ).setEnabled(this.parent != null)
            ;(
                this.starProperties as PropertiesImplementation<OutProperties>
            ).setEnabled(this.parent != null)
        }
        ...
        this.properties.setLayersWithConditionals(
        ...
        abortableEffect(() => {
            ...
            const cleanup = parentProprties?.subscribePropertyKeys((key) => {
                ...
                const signal =
                    parentProprties.signal[
                        key as keyof typeof parentProprties.signal
                    ]
        ...
        this.starProperties = new PropertiesImplementation<OutProperties>(
            allAliases,
            conditionals,
        )
        ...
        abortableEffect(() => {
            const parentStarProprties =
                this.parentContainer.value?.starProperties ?? globalProperties
            const layerIndex = getLayerIndex({ type: 'star-inheritance' })
            const cleanup = parentStarProprties?.subscribePropertyKeys(
                (key) => {
                    const signal =
                        parentStarProprties.signal[
                            key as keyof typeof parentStarProprties.signal
                        ]
        ...
        this.isVisible = computedIsVisible(
            this,
            this.isClipped,
            this.properties,
        )
    ...
    raycast(raycaster: Raycaster, intersects: Intersection[]): unknown {
        ...
        sphereHelper
            .copy(this.boundingSphere)
            .applyMatrix4(worldToGlobalMatrixHelper)
