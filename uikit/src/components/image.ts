...
export class Image<
    OutProperties extends ImageOutProperties<unknown> = ImageOutProperties<
        string | Texture
    >,
    ...
    constructor(
        ...
        setupOrderInfo(
            ...
            computed(() =>
                this.parentContainer.value == null
                    ? null
                    : this.parentContainer.value.orderInfo.value,
            ),
            this.abortSignal,
        )
        ...
        if (inputConfig?.loadTexture ?? true) {
            loadResourceWithParams(
                ...
                cleanupTexture,
                this.abortSignal,
                this.properties.signal.src as Signal<
                    string | Texture | undefined
                >,
            )
        }
        ...
        abortableEffect(() => {
            ;(this.material as Partial<WebGPUImageMaterial>).setTexture?.(
                this.texture.value,
            )
        ...
        abortableEffect(() => {
            const materialClass = resolvePanelMaterialClassProperty(
                this.properties.value.panelMaterialClass,
            )
            const material = getImageRenderBackend(
                this.root.value.backend,
            ).createImageMaterial(materialClass, data, this.texture.value)
            ...
            ;(material as Partial<WebGPUImageMaterial>).syncClipping?.(
                clippingData,
            )
        ...
        abortableEffect(() => {
            ...
            if (textureRatio > boundsRatioValue) {
                ...
                texture.matrix
                    .translate(
                        0,
                        -(0.5 * (textureRatio - boundsRatioValue)) /
                            textureRatio,
                    )
        ...
        abortableEffect(() => {
            ...
            ;(this.material as Partial<WebGPUImageMaterial>).syncClipping?.(
                clippingData,
            )
            this.root.peek().requestRender?.()
        }, this.abortSignal)
...
function transformInsideBorder(
    ...
    texture.matrix
        .translate(
            -1 + (left + width) / outerWidth,
            -1 + (top + height) / outerHeight,
        )
...
function cleanupTexture(
    texture: (Texture & { disposable?: boolean }) | undefined,
): void {
