...
import { Matrix4, Vector2Tuple } from 'three/webgpu'
...
export class InstancedText {
    ...
    constructor(
        ...
        private properties: Properties<
            AdditionalTextDefaults & BaseOutProperties
        >,
        ...
        private caretTransformation:
            | Signal<CaretTransformation | undefined>
            | undefined,
    ) {
        this.unsubscribeInitialList = [
            effect(() => {
                if (
                    !isVisible.value ||
                    toAbsoluteNumber(this.properties.value.opacity, () => 1) <
                        0.01
                ) {
                    this.hide()
                    return
    ...
    public getCharIndex(
        ...
        const lineIndex = Math.floor(
            y / -getOffsetToNextLine(layout.lineHeight),
        )
        ...
        const whitespaceWidth =
            layout.font.getGlyphInfo(' ').xadvance * layout.fontSize
    ...
    private updateSelectionBoxes(
        ...
        if (
            this.caretTransformation == null ||
            this.selectionTransformations == null
        ) {
            return
        }
        ...
        if (endCharIndexExcl <= startCharIndexIncl) {
            ...
            this.caretTransformation.value = {
                position: [x, y - layout.fontSize / 2],
        ...
        if (start.lineIndex === end.lineIndex) {
            this.selectionTransformations.value = [
                this.computeSelectionTransformation(
                    start.lineIndex,
                    start.x,
                    end.x,
                    layout,
    ...
    private show() {
        ...
        this.unsubscribeShowList.push(
            ...
            effect(() => {
                ...
                const opacity = toAbsoluteNumber(
                    this.properties.value.opacity,
                    () => 1,
                )
                traverseGlyphs(this.glyphLines, (glyph) =>
                    glyph.updateColor(color ?? 0, opacity),
                )
            }),
            effect(() => {
                ...
                let y =
                    getYOffset(layout, this.properties.value.verticalAlign) -
                    layout.availableHeight / 2
                ...
                for (let lineIndex = 0; lineIndex < linesLength; lineIndex++) {
                    ...
                    let offsetPerWhitespace =
                        textAlign === 'justify'
                            ? (availableWidth - nonWhitespaceWidth) /
                              whitespacesBetween
                            : 0
                    let x =
                        getXOffset(
                    ...
                    for (
                        ...
                        const glyphIndex =
                            charIndex - firstNonWhitespaceCharIndex
                        ...
                        glyph.updateGlyphAndTransformation(
                            ...
                            -(
                                y +
                                getGlyphOffsetY(fontSize, lineHeight, glyphInfo)
                            ),
                            fontSize,
                            pixelSize,
                        )
...
function getYOffset(
    ...
    switch (verticalAlign) {
        ...
        case 'bottom':
            return (
                layout.availableHeight -
                getGlyphLayoutHeight(layout.lines.length, layout.lineHeight)
            )
        default:
            return 0
    }
