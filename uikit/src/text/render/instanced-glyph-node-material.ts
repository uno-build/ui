...
function median(r: ReturnType<typeof float>, g: ReturnType<typeof float>, b: ReturnType<typeof float>) {
...
export class WebGPUInstancedGlyphMaterial extends MeshBasicNodeMaterial {
  ...
  constructor(font: Font) {
    ...
    const glyphSurface = Fn(() => {
      const glyphLocalPosition = varyingProperty('vec3', 'glyphLocalPosition')
      const glyphUv = uv()
      const fontUv = instanceUVOffset.xy.add(glyphUv.mul(instanceUVOffset.zw)).toVar()
