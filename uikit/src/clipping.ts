...
export class ClippingRect {
    ...
    constructor(
        ...
        this.originalCenter = new Vector3(centerX, centerY, 0).applyMatrix4(
            globalMatrix,
        )
        this.facePlane = new Plane(new Vector3(0, 0, 1), 0).applyMatrix4(
            globalMatrix,
        )
    ...
    min({ planes }: ClippingRect): this {
        for (let i = 0; i < 4; i++) {
            ...
            helperPlane.normal
                .crossVectors(p1.normal, p2.normal)
                .normalize()
                .cross(p1.normal)
                .negate()
        ...
        while (
            helperPlanes[indexOffset]!.normal.dot(firstPlaneNormal) > dotLt45deg
        ) {
            break
        }
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (
                helperPlanes[otherPlaneIndex]!.distanceToPoint(
                    this.originalCenter,
...
export const NoClippingPlane = new Plane(
    new Vector3(-1, 0, 0),
...
export function createGlobalClippingPlanes(component: Component) {
    const getGlobalMatrix = () =>
        component.root.peek().component.parent?.matrixWorld
    const planes = new Array(4)
        ...
        .map<Plane>(
            (_, i) =>
                new RelativePlane(
                    () =>
                        component.parentContainer.peek()?.clippingRect.value
                            ?.planes[i],
                    getGlobalMatrix,
                ),
        )
