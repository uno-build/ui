...
export function computedAnyAncestorScrollable(
    parentSignal: Signal<Container | undefined>,
) {
    return computed(() => {
        const parent = parentSignal.value
        const [ancestorX, ancestorY] = parent?.anyAncestorScrollable?.value ?? [
            false,
            false,
        ]
...
export function setupScrollHandlers(
    ...
    const isScrollable = computed(
        () =>
            container.scrollable.value.some((scrollable) => scrollable) ??
            false,
    )
    ...
    abortableEffect(() => {
        ...
        target.value = {
            onPointerDown: (event) => {
                ...
                if (
                    event.pointerId == null ||
                    (ponterIsMouse && scrollbarAxisIndex == null)
                ) {
                    return
                }
            ...
            onPointerMove: (event) => {
                ...
                const prevInteraction = container.downPointerMap.get(
                    event.pointerId,
                )
                ...
                distanceHelper
                    ...
                    .sub(prevInteraction.localPoint)
...
function scroll(
    ...
    const [ancestorScrollableX, ancestorScrollableY] = container
        .anyAncestorScrollable?.value ?? [false, false]
    newX = computeScroll(
        x,
        maxX,
        deltaX,
        enableRubberBand && !ancestorScrollableX,
    )
    newY = computeScroll(
        y,
        maxY,
        deltaY,
        enableRubberBand && !ancestorScrollableY,
    )
...
export function setupScroll(container: Container) {
    const onFrame = (delta: number) => {
        if (
            container.downPointerMap.size > 0 ||
            container.scrollPosition.value == null
        ) {
            return
        }
        ...
        if (
            Math.abs(container.scrollVelocity.x) < 0.01 /** 10 px per second */
        ) {
        ...
        if (
            Math.abs(container.scrollVelocity.y) < 0.01 /** 10 px per second */
        ) {
...
export function setupScrollbars(
    ...
    const borderInset = computedBorderInset(
        container.properties,
...
function setupScrollbar(
    ...
    const scrollbarPosition = computed(
        () =>
            (scrollbarTransformation.value?.slice(0, 2) ?? [
                0, 0,
            ]) as Vector2Tuple,
    )
    const scrollbarSize = computed(
        () =>
            (scrollbarTransformation.value?.slice(2, 4) ?? [
                0, 0,
            ]) as Vector2Tuple,
    )
...
function computeScrollbarTransformation(
    ...
    const primarySizeWithoutBorder =
        ...
        borderInset[endInsetIndex]! -
        borderInset[endInsetIndex + 2]!
    ...
    const primaryMaxScrollbarPosition =
        primarySizeWithoutBorder - primaryScrollbarSize
    ...
    result[primaryAxisIndex] =
        ...
        primaryMaxScrollbarPosition *
            clamp(primaryScrollPosition / primaryMaxScrollPosition, 0, 1)
    result[invertedIndex] =
        size[invertedIndex]! * 0.5 -
        secondaryScrollbarSize * 0.5 -
        borderInset[invertedIndex + 1]!
