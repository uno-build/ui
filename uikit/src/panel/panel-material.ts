...
export function getDefaultPanelMaterialConfig() {
    if (defaultPanelMaterialConfig == null) {
        const defaultPanelMaterialKeys = {} as {
            [Key in keyof typeof defaultDefaults]: string
        }
        for (const key in defaultDefaults) {
            defaultPanelMaterialKeys[key as keyof typeof defaultDefaults] = key
        }
        defaultPanelMaterialConfig = createPanelMaterialConfig(
            ...
            return computed(() => {
                ...
                const backgroundColor =
                    keys.backgroundColor == null
                        ? defaults.backgroundColor
                        : (properties.value[
                              keys.backgroundColor as 'backgroundColor'
                          ] ?? defaults.backgroundColor)
                const borderColor =
                    keys.borderColor == null
                        ? defaults.borderColor
                        : (properties.value[
                              keys.borderColor as 'borderColor'
                          ] ?? defaults.borderColor)

                const opacity = toAbsoluteNumber(
                    properties.value.opacity ?? defaultOpacity,
                    () => 1,
                )
                ...
                const backgroundVisible =
                    width > 0 && height > 0 && colorArrayHelper[3]! > 0
                ...
                const borderVisible =
                    borderInset.value.some((s) => s > 0) &&
                    colorArrayHelper[3]! > 0
...
export function writeColor(
    ...
    if (Array.isArray(color)) {
        ...
        target[offset + 3] =
            (color.length === 3 ? 1 : target[offset + 3]!) * opacity
    } else if (color === 'transparent') {
        target.fill(0, offset, offset + 4)
    } else if (
        typeof color === 'string' &&
        (match = color.match(rgbaRegex)) != null
    ) {
        for (let i = 0; i < 3; i++) {
...
export type PanelMaterialInfo =
    | { type: 'instanced' }
    | { type: 'normal'; data: Float32Array }

export function createPanelMaterial<T extends Constructor<Material>>(
    MaterialClass: T,
...
export const instancedPanelDepthMaterial = new PanelDepthMaterial({
    type: 'instanced',
})
export const instancedPanelDistanceMaterial = new PanelDistanceMaterial({
    type: 'instanced',
})

function compilePanelDepthMaterial(
    parameters: WebGLProgramParametersWithUniforms,
    instanced: boolean,
) {
...
function compilePanelClippingMaterial(
    parameters: WebGLProgramParametersWithUniforms,
    instanced: boolean,
) {
    ...
    parameters.fragmentShader =
        getFragmentShaderPrefix(instanced) + parameters.fragmentShader
...
function getFragmentOpacityCode(
    instanced: boolean,
    existingOpacity: string | undefined,
) {
