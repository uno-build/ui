...
export function resolvePanelMaterialClassProperty(
    input: NonNullable<PanelGroupProperties['panelMaterialClass']>,
) {
...
export function computedPanelGroupDependencies(properties: Properties) {
    return computed<Required<PanelGroupProperties>>(() => {
        return {
            panelMaterialClass: resolvePanelMaterialClassProperty(
                properties.value.panelMaterialClass,
            ),
...
export class InstancedPanelGroup {
    ...
    constructor(
        ...
        this.instanceMaterial = renderBackend.createPanelMaterial(
            materialClass,
            { type: 'instanced' },
        )
    ...
    private requestUpdate(time: number): void {
        ...
        this.nextUpdateTimeoutRef = setTimeout(
            this.requestUpdateNextFrame.bind(this),
            time,
        )
    }
    ...
    private resize(): void {
        ...
        this.instanceData = createDynamicFloat32InstancedAttribute(
            16,
            this.bufferElementSize,
            this.instanceData,
        )
