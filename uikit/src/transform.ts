...
import { Euler, Matrix4, Quaternion, Vector3, Vector3Tuple } from 'three/webgpu'
...
function toQuaternion([x, y, z]: Vector3Tuple): Quaternion {
    return quaternionHelper.setFromEuler(
        eulerHelper.set(x * toRad, y * toRad, z * toRad),
    )
}
...
export function computedTransformMatrix({
    ...
    return computed(() => {
        ...
        const result = new Matrix4().makeTranslation(
            x * pixelSize,
            y * pixelSize,
            0,
        )
