import { Object3D, Vector2Tuple } from 'three/webgpu'
...
export class FlexNode {
    ...
    commit(parentDirection: FlexDirection): void {
        ...
        const parentDirectionVertical =
            ...
            parentDirection === FlexDirection.ColumnReverse
        if (
            this.customLayouting != null &&
            this.component.properties.peek()[
                parentDirectionVertical ? 'minHeight' : 'minWidth'
            ] === undefined
        ) {
            this.yogaNode[
                parentDirectionVertical ? 'setMinHeight' : 'setMinWidth'
            ](
                parentDirectionVertical
                    ? this.customLayouting.minHeight
                    : this.customLayouting.minWidth,
            )
        }
        ...
        if (this.component.properties.peek().flexShrink == null) {
            ...
            this.yogaNode.setFlexShrink(
                hasHeight && parentDirectionVertical ? 0 : undefined,
            )
        }
        /** ---- END ---- */
        ...
        while (correctChild != null || oldChildNode != null) {
            if (
                ...
                yogaNodeEqual(
                    oldChildNode,
                    assertNodeNotNull(correctChild.yogaNode),
                )
            ) {
    ...
    updateMeasurements(
        ...
        if (this.yogaNode == null) {
            throw new Error(
                `update measurements cannot be called without a yoga node`,
            )
        }
        ...
        for (let i = 0; i < childrenLength; i++) {
            const [contentWidth, contentHeight] = this.children[
                i
            ]!.updateMeasurements(displayed, width, height)
        ...
        const overflowVisible =
            this.component.overflow.value === Overflow.Visible
...
export function setMeasureFunc(node: Node, func: MeasureFunction | undefined) {
    ...
    node.setMeasureFunc((width, widthMode, height, heightMode) => {
        ...
        result.width =
            Math.ceil(result.width * PointScaleFactor) / PointScaleFactor
        result.height =
            Math.ceil(result.height * PointScaleFactor) / PointScaleFactor
        return result
    })
    node.markDirty()
}
...
function assertNodeNotNull<T>(val: T | undefined): T {
    if (val == null) {
        throw new Error(
            `commit cannot be called with a children that miss a yoga node`,
        )
    }
    return val
}
