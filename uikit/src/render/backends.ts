...
const webglImageRenderBackend: ImageRenderBackend = {
    backend: 'webgl',
    capabilities: webglCapabilities,
    createImageMaterial: (materialClass, data) =>
        createPanelMaterial(materialClass, { type: 'normal', data }),
}
...
const experimentalWebGPUImageRenderBackend: ImageRenderBackend = {
    backend: 'webgpu',
    capabilities: webgpuCapabilities,
    createImageMaterial: (materialClass, data, texture) =>
        createWebGPUImageMaterial(materialClass, data, texture),
}

export function getDefaultBackendCapabilities(
    backend: RendererBackend,
): BackendCapabilities {
    return backend === 'webgpu' ? webgpuCapabilities : webglCapabilities
}

export function getPanelRenderBackend(
    backend: RendererBackend,
): PanelRenderBackend {
    return backend === 'webgpu'
        ? experimentalWebGPUPanelRenderBackend
        : webglPanelRenderBackend
}

export function getTextRenderBackend(
    backend: RendererBackend,
): TextRenderBackend {
    return backend === 'webgpu'
        ? experimentalWebGPUTextRenderBackend
        : webglTextRenderBackend
}

export function getImageRenderBackend(
    backend: RendererBackend,
): ImageRenderBackend {
    return backend === 'webgpu'
        ? experimentalWebGPUImageRenderBackend
        : webglImageRenderBackend
}
