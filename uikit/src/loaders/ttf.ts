...
import { FileLoader, Loader, LoadingManager } from 'three/webgpu'

export type MSDFResult = GenerateFontResult

export interface TTFLoaderOptions extends Partial<
...
function normalizeInput(input: TTFInput): NormalizedFont[] {
    ...
    return items.map((item, index) => {
        ...
        if (!item.url)
            throw new Error(
                `TTFLoader: Font at index ${index} is missing 'url'`,
            )
...
export class TTFLoader extends Loader<MSDFResult, TTFInput> {
    ...
    private async _loadFontFiles(
        ...
        const loadPromises = fonts.map((font) =>
            loader.loadAsync(font.url, onProgress),
        )
    ...
    private async _generate(
        ...
        try {
            ...
            throw new Error(
                `TTFLoader: MSDF generation failed for ${urls}: ${err instanceof Error ? err.message : err}`,
            )
        } finally {
            generator.dispose()
        }
