...
export function buildRootContext(
    ...
    abortableEffect(() => {
        ...
        root.value.requestCalculateLayout =
            createDeferredRequestLayoutCalculation(root.value, component)

        const onFrame = () =>
            void (root.value.reversePainterSortStableCache = undefined)

        root.value.onFrameSet.add(onFrame)
        abortController.signal.addEventListener('abort', () =>
            root.value.onFrameSet.delete(onFrame),
        )
...
function createRootContext(
    ...
    const ctx: Omit<RootContext, 'glyphGroupManager' | 'panelGroupManager'> = {
        backend,
        backendCapabilities: {
            ...getDefaultBackendCapabilities(backend),
...
export function buildRootMatrix(
