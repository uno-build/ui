import type { ResolvedWebGPUImage } from './contracts'
...
export class ImageManager {
    images: Map<string, ManagedAtlasImage> = new Map()
    ...
    private atlas_texture: GPUTexture | null

    private atlas_layer_count: number = 1

    private atlas_texture_layer_count: number = 1
    ...
    imageUpload(src: string, image: ResolvedWebGPUImage): ManagedAtlasImage {
        ...
        if (image.width > this.atlas_size || image.height > this.atlas_size) {
            throw new Error(
                `Image "${src}" is ${image.width}x${image.height}, which exceeds the ${this.atlas_size}x${this.atlas_size} UI atlas layer size.`,
            )
        }
    ...
    private allocateAtlasRect(width: any, height: any) {
        for (const atlas_layer of this.atlas_layers) {
    ...
    private tryAllocateAtlasRect(atlas_layer: any, width: any, height: any) {
    ...
    private createAtlasLayer(layer: any): AtlasLayer {
        return {
            layer,
    ...
    private createAtlasTexture(layer_count: any) {
    ...
    private getAtlasTexture(): GPUTexture {
        this.atlas_texture ??= this.createAtlasTexture(this.atlas_texture_layer_count)
        return this.atlas_texture
    }

    private copyImagePadding(image: any, texture: any, x: any, y: any, layer: any) {
    ...
    private createAtlasUvRect(x: any, y: any, width: any, height: any): [number, number, number, number] {
