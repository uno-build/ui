...
import type { StyleUpdate, ResolvedStyle, ComputedLayout, StyleName, StyleContext } from '../style/types'
...
> {
    ...
    private viewport_width: any

    private viewport_height: any

    private root_size: number = ROOT_SIZE

    private layouter: any

    private pipeline: any

    private bind_group: any

    private image_sampler: any

    private image_manager: ImageManager | null

    private image_texture_version: any

    private font_texture_version: any

    private position_buffer: any

    private viewport_buffer: any

    private viewport_data: Float32Array<ArrayBuffer> = new Float32Array(4)

    private command_pool: any

    private command_count: number = 0

    private panel_data_pool: any

    private glyph_data_pool: any

    private text_run_pool: any

    private records: Map<any, any> | null = new Map()

    private prepared_texts: WeakMap<WeakKey, any> | null = new WeakMap()
    ...
    async init(): Promise<RendererWebGPUOutput> {
    ...
    createElement(node: Node<undefined>): undefined {
        if (node.id === 0) {
            this.root_node = node
        }
    ...
    getChildIndex(node: Node<undefined>): number {
        return this.layouter.getChildIndex(node)
    }

    prepareLayout(nodes_created: Set<Node<undefined>>, operations: Operations<undefined>): boolean {
        const fonts_changed = operations.items.some(({ op }: Operation<undefined>) => op === OPERATIONS.RESOURCE_FONT)
    ...
    initializeTextNode(node: Node<undefined>) {
        this.layouter.setMeasureFunction(node, (width: any, width_mode: any, height: any, height_mode: any) =>
            this.getTextMeasure(node, width, width_mode, height, height_mode),
        )
    }

    invalidateTextNode(node: Node<undefined>) {
    ...
    protected insertChild(parent: Node<undefined>, node: Node<undefined>, child_index: number) {
        this.layouter.insertChild(parent, node, child_index)
    }

    detachChild(parent: Node<undefined>, node: Node<undefined>, release_subtree: boolean | undefined = true) {
    ...
    destroyNode(node: Node<undefined>) {
        this.layouter.destroyNode(node)
        this.releaseRecord(node)
    }

    updateStyle(node: Node<undefined>, resolved_style: StyleUpdate) {
    ...
    getLayout(node: Node<undefined>): ComputedLayout {
        return this.layouter.getLayout(node)
    }

    beforeUpdate(nodes: Node<undefined>[], operations: Operations<undefined>) {
        if (operations.needUpdateLayout()) {
            this.layouter.calculate(this.viewport_width, this.viewport_height)
        }
    }

    update(nodes: Node<undefined>[], operations: Operations<undefined>) {
    ...
    private updateResolvedStyle(node: any, style: any) {
    ...
    private getNodeContentSize(node: any) {
        if (!node.hasTextContent()) {
            return null
        }
    ...
    private createRenderPlan(nodes: any, operations: any) {
        let global_parts = 0
        ...
        const addRecord = (node: any, parts: any) => {
            if (parts !== 0) {
                record_parts.set(node, (record_parts.get(node) ?? 0) | parts)
            }
        }
        const addSubtree = (node: any, parts: any) => {
    ...
    private updateRecord(node: any, record: any, parts: any, getNodeMetrics: any) {
    ...
    private releaseRecord(node: any) {
    ...
    private releaseSubtreeRecords(node: any) {
        this.releaseRecord(node)
        for (const child of node.children) {
    ...
    private releasePanel(record: any) {
        if (record.panel_slot !== -1) {
    ...
    private releaseText(record: any) {
        if (record.run_slot !== -1) {
    ...
    private collectTextInstanceData(node: any, record: any, parts: any, getNodeMetrics: any) {
        ...
        if (text_shadow_data.some((value: any, index: number) => value !== record.text_shadow[index])) {
            parts |= RECORD_GLYPHS
        }
    ...
    private getPreparedText(node: any, font: any, font_size: any) {
    ...
    private updateBuffers(update_viewport: any) {
