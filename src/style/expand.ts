...
function parseCircular(value: string, resolve) {
...
function parseFlowRelativePosition(value: string, resolve) {
...
const textDecorationLineValues = new Set(
    GLOBAL_VALUES.concat(['none', 'underline', 'overline', 'line-through', 'blink']),
)
const textDecorationStyleValues = new Set(
    GLOBAL_VALUES.concat(['solid', 'double', 'dotted', 'dashed', 'wavy']),
)

function parseTextDecoration(value: string) {
...
function parseOverflow(value: string) {
...
const flexDirectionValues = new Set(
    GLOBAL_VALUES.concat(['row', 'row-reverse', 'column', 'column-reverse']),
)
const flexWrapValues = new Set(GLOBAL_VALUES.concat(['nowrap', 'wrap', 'reverse']))

function parseFlexFlow(value: string) {
...
function parsePlaceContent(value: string) {
...
function parsePlaceItems(value: string) {
...
function parsePlaceSelf(value: string) {
...
const extractTransition = (value: string) => {
    const values = splitShorthand(value)
...
function expand(property: string, value: string) {
    ...
    if (property === 'flexFlow') {
        return parseFlexFlow(value)
    }

    if (property === 'placeContent') {
        return parsePlaceContent(value)
    }

    if (property === 'placeItems') {
        return parsePlaceItems(value)
    }

    if (property === 'placeSelf') {
        return parsePlaceSelf(value)
    }
    ...
    if (flowRelativePositionExpand[property]) {
        return parseFlowRelativePosition(value, flowRelativePositionExpand[property])
    }
}
