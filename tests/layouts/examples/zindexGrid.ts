export default function createZIndexLayout({ ui, rendererName }) {
    ...
    Container.setStyle('flexWrap', 'wrap')
    Container.setStyle('alignContent', 'flex-start')
    Container.setStyle('flex', '1')
    Container.setStyle('padding', `${PADDING}px`)
    ui.root.add(Container)

    const Cell1 = createNode(ui, createCell({
    ...
    const Label1 = createNode(ui, createLabel({
        backgroundColor: '#f00',
    }))
    Cell1.add(Label1)

    const Cell2 = createNode(ui, createCell({
    ...
    const Label2 = createNode(ui, createLabel({
        backgroundColor: '#0f0',
    }))
    Cell2.add(Label2)

    const Cell3 = createNode(ui, createCell({
    ...
    const Label3 = createNode(ui, createLabel({
        backgroundColor: '#00f',
    }))
    Cell3.add(Label3)

    const Cell4 = createNode(ui, createCell({
    ...
    const Label4 = createNode(ui, createLabel({
        backgroundColor: '#ff0',
        position: 'absolute',
    }))
    Cell4.add(Label4)
...
function createNode(ui, styles) {
    ...
    for (const name of Object.keys(styles)) {
        node.setStyle(name, styles[name])
    }
    return node
}
