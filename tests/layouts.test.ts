...
test('Layout: backgroundImage updates and clears with unset', async ({ page }) => {
    ...
    const results = await page.evaluate(
        async ({ layoutRunnerUrl, loadImageUrl, renderers, uiUrl }) => {
            ...
            function readBackgroundImage(canvas, renderer, node, inspect_dom_paint) {
                if (inspect_dom_paint === false) {
                    return renderer.imageList().find(({ nodes }) => nodes.has(node))?.src ?? 'none'
                }
            ...
            for (const rendererName of renderers) {
                ...
                ui.update()
                ui.draw()
                const first = readBackgroundImage(canvas, renderer, node, setup.inspectDomPaint)

                node.style('backgroundImage', asset_coin.src)
                ui.update()
                ui.draw()
                const second = readBackgroundImage(canvas, renderer, node, setup.inspectDomPaint)

                node.style('backgroundImage', 'unset')
                ui.update()
                ui.draw()
                const unset = readBackgroundImage(canvas, renderer, node, setup.inspectDomPaint)
