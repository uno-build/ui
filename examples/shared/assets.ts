export const FONT_NAME_POPPINS = 'Poppins-Regular'
export const FONT_NAME_CHANGA = 'ChangaOne-Regular'
export const FONT_NAME_SUPERCELL = 'Supercell-Magic'
export const FONT_NAME_NOUGAT = 'Nougat-ExtraBlack'
export const FONT_NAME_BANGERS = 'Bangers-Regular'

export async function loadFont(font_name, { loadImage, loadJson }) {
    const image = await loadImage(new URL(`../assets/fonts/${font_name}.png`, import.meta.url).href)
    const data = await loadJson(new URL(`../assets/fonts/${font_name}.json`, import.meta.url).href)
...
export async function loadAssets({ loadImage, loadJson }) {
    ...
    const font3 = await loadFont(FONT_NAME_SUPERCELL, { loadImage, loadJson })
...
export function registerAssets({ resources, assets }) {
    ...
    resources.registerFont(FONT_NAME_CHANGA, font)
    resources.registerFont(FONT_NAME_POPPINS, font2)
    resources.registerFont(FONT_NAME_SUPERCELL, font3)
}
