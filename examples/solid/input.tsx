...
export function SolidInput() {
    let nickname_ref
    const [focused, setFocused] = createSignal(null)
            ...
            <View style={CARD_STYLE}>
                ...
                <View style={FOOTER_STYLE}>
                    <Text style={STATUS_STYLE}>
                        {focused() === null ? 'Tap a field to edit' : `Editing ${focused()}`}
                    </Text>
...
export default function createSolidInput({ ui, resources }) {
    return Promise.all([
        loadImage(`/${COIN_SRC}`),
