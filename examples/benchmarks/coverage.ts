import { FONT_NAME1, FONT_NAME2, FONT_NAME5 } from '../shared/assets'
import { STYLE, resolveStyle } from '../../src/style'
import { FEATURES } from '../../src/renderer/utils/render-metrics'

export const FONT_NAMES = [FONT_NAME2, FONT_NAME1, FONT_NAME5]
...
function defineCases(image_sources) {
    return [
        ...
        {
            id: 'border-shorthand',
            features: ['panel', 'border', 'border_radius'],
            styles: {
                border: '3px solid #47caba',
                borderRadius: '12px',
            },
        },
        {
            ...
            styles: {
                ...
                borderTopColor: '#ff9878',
                borderRightColor: '#f4d56e',
                borderBottomColor: '#74dcc0',
                borderLeftColor: '#92aaff',
                borderTopWidth: '2px',
        ...
        {
            id: 'image-cover',
            features: ['panel', 'background_image'],
            styles: {
                backgroundImage: image_sources[0],
        ...
        },
        {
            ...
            features: ['panel', 'background_image'],
            styles: {
                backgroundImage: image_sources[1],
        ...
        },
        {
            ...
            features: ['panel', 'background_image'],
            styles: {
                backgroundImage: image_sources[2],
        ...
        },
        {
            ...
            styles: {
                backgroundImage: image_sources[1],
                backgroundSizeWidth: '18px',
                backgroundSizeHeight: '50%',
        ...
        },
        {
            id: 'stroke-small',
            text: TEXT_CORPUS[0],
            features: ['panel', 'text', 'text_stroke'],
            expect: { text_stroke_multisampling: false },
            styles: {
                fontFamily: FONT_NAMES[0],
                fontSize: '24px',
                textStroke: '0.1px #ffdc8b',
                color: '#ffffff',
                textAlign: 'left',
            },
        },
        {
            ...
            text: TEXT_CORPUS[1],
            features: ['panel', 'text', 'text_stroke'],
            expect: { text_stroke_multisampling: true },
            styles: {
                fontFamily: FONT_NAMES[1],
                fontSize: '24px',
                textStroke: '12px #6354b4',
                color: '#ffffff',
                textAlign: 'right',
            },
        },
        {
            id: 'text-shadow',
            text: TEXT_CORPUS[2],
            features: ['panel', 'text', 'text_shadow'],
            styles: {
                fontFamily: FONT_NAMES[2],
                fontSize: '22px',
                textShadow: '2px 3px 2px #101020',
                textAlign: 'center',
            },
        },
        {
        ...
        },
        {
            ...
            children: [{ width: '280px', height: '120px' }],
        },
        {
            id: 'nested-scroll',
            features: ['panel'],
            styles: { overflowX: 'scroll', overflowY: 'scroll' },
            children: [{ width: '280px', height: '120px', flexShrink: '0', overflow: 'hidden' }],
            scroll: true,
        },
        {
            id: 'inherited-opacity',
            features: ['panel'],
            cell_styles: { opacity: '0.5' },
            styles: { opacity: '0.5' },
            expect: { opacity: 0.25 },
        },
        {
            ...
            features: [],
            visible: false,
            styles: { display: 'none', fontFamily: FONT_NAMES[2] },
            expect: { panel: false, text: false },
        },
        {
            id: 'effects-disabled',
            features: ['panel'],
...
export function getCoverageManifest(image_sources) {
    ...
    const missing_styles = Object.values(STYLE)
        .map((style) => style.name)
        .filter((name) => !applied_styles.has(name))
...
export function createCoverage({ ui, resources, image_sources, setViewport }) {
    ...
    if (manifest.missing_styles.length || manifest.missing_features.length) {
        throw new Error(
            `Benchmark coverage missing: ${[...manifest.missing_styles, ...manifest.missing_features].join(', ')}`,
        )
    }
    ...
    for (const [index, definition] of defineCases(image_sources).entries()) {
        const cell = createNode(root, {
            ...CELL_STYLE,
            left: `${12 + (index % 5) * 250}px`,
            top: `${12 + Math.floor(index / 5) * 138}px`,
