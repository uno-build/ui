...
export function OctaneTodo({
    ...
    const last_label_click = useRef({ id: null, time: 0 })
    ...
    const [hovered_id, setHoveredId] = useState(null)
    ...
    function addTodo(title) {
        ...
        setTodos((current) => [...current, { id: next_id.current++, title: next_title, completed: false }])
        setDraft('')
    }
...
export default function createOctaneTodo({ ui, resources }) {
    return Promise.all([
        loadImage(`/${COIN_SRC}`),
        ...
        const renderer = registerRootComponent(OctaneTodo, { ui })
        renderer.render({})
    })
}
